...
def predict(train_df_shape, data):
    data = get_data(data)

    model_path = 'rl/output/target_model/model.pt'
    trading_agent = Agent_DQN(train_df_shape, NUM_EPISODES, window_size=48, gamma=0.95, update_interval=96, lr=0.01, min_epsilon=0.02)
    ...
    state = trading_agent.get_state(0, data)
    ...
    for t in range(len(data) - 1):
        ...
        next_state = trading_agent.get_state(t + 1, data)
        reward = trading_agent.trade(t, action, data, data, INITIAL_INVESTMENT, trading_fee_rate = 0.01)
    ...
    with open('/Users/alexandrnikitin/Pictures/Trading_bot_RL/crypto-bot/RL/test_output_files/testing_scores.out', 'a') as f:
        f.write(f"TESTING (runtime: {time() - t0})   |  Portfolio Value is {round(testing_mem['Portfolio Value'][-1], 3)}\n")

    with open('/Users/alexandrnikitin/Pictures/Trading_bot_RL/crypto-bot/RL/test_output_files/testing_mem.json', 'w') as f:
